...
public class PageControlViewController: UIViewController, UICollectionViewDelegate {
    ...
    func setPage(_ newPage: UIViewController?){
    ...
    public override func viewDidAppear(_ animated: Bool) {
        ...
        self.data = [:]
        print(currentPos)
        if(isFirstController == true && currentPos == 0) {
            self.currentPos = 0
        } else {
            self.currentPos = 1
        }
    ...
    open func updateData(){
        ...
        if self.currentPos == 1 {
            let vc : ReferralLeaderboardVC  = self.data![0] as! ReferralLeaderboardVC
            vc.hideCloseBtn(isHidden: true)
        }
    ...
    func nextPage(){
        if let dataSource = self.dataSource {
            let vc : ReferralLeaderboardVC  = self.data![0] as! ReferralLeaderboardVC
            vc.hideCloseBtn(isHidden: true)
            if (self.currentPos + 1) < self.count {
                currentPos += 1
                if let page = dataSource.pageControl(self, cellAtRow: currentPos) {
                    self.setPage(page)
                    UIView.animate(withDuration: self.animationSpeed, animations: {
