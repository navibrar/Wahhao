...
class OrderRelatedItemCell: UITableViewCell {
    ...
    @IBOutlet weak var orderProduct_Price : UILabel!
    ...
    func configureCell(item: Order)  {
        ...
        totalPrice = (item.customer_final_price as NSString).floatValue
        let totalPriceStr = formatPriceToTwoDecimalPlace(amount: totalPrice)
        orderProduct_Price.text = totalPriceStr
