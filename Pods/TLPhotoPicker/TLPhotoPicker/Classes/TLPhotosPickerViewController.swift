...
public struct TLPhotosPickerConfigure {
   ...
   // public var cameraBgColor = UIColor(red: 221/255, green: 223/255, blue: 226/255, alpha: 1)
    public var cameraBgColor = #colorLiteral(red: 0.06666666667, green: 0.137254902, blue: 0.2549019608, alpha: 1)
...
extension TLPhotosPickerViewController {
    ...
    fileprivate func centerAtRect(image: UIImage?, rect: CGRect, bgColor: UIColor = UIColor.red) -> UIImage? {
...
extension TLPhotosPickerViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    fileprivate func showCamera() {
        ...
        //Mark:- Customization
        if self.configure.mediaType == .video {
    ...
    open func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = (info[.originalImage] as? UIImage) {
            ...
            var placeholderAsset: PHObjectPlaceholder? = nil
            PHPhotoLibrary.shared().performChanges({
                let newAssetRequest = PHAssetChangeRequest.creationRequestForAssetFromVideo(atFileURL: info[UIImagePickerController.InfoKey.mediaURL] as! URL)
                placeholderAsset = newAssetRequest?.placeholderForCreatedAsset
...
extension TLPhotosPickerViewController: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDataSourcePrefetching {
    ...
    open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if phAsset.mediaType == .video {
            if phAsset.duration > 31 {
                let alert = UIAlertController(title: "", message: "Video duration is limited to 30 seconds", preferredStyle: .alert)
            ...
            /*asset.videoSize(completion: { (size) in
             let calculatedSize = (Double(size)/1000.0)/1000.0
